 ...
 *   [{ month: '2025-12', Tithe: 12822, Offertory: 5979, ... }, ...]
 */
...
export default function IncomeByCategory() {
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      const res = await getIncomeByCategoryReport({
        from_date: fromDate,
        to_date: toDate,
      })
      setData(res.data)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p style={{color:'#44474f',marginTop:'4px'}}>
          Monthly aggregation across all income categories for council review
        </p>
      </div>
      ...
      {data && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <SummaryCard label="Grand Total" value={formatGHS(data.summary.grand_total)}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                <tr style={{borderTop:'2px solid var(--color-navy)',backgroundColor:'#f8f9fa'}}>
                  <td className="px-6 py-3 font-bold" style={{color:'var(--color-navy)'}}>Grand Total</td>
