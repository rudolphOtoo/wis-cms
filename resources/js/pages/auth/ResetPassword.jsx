    ...
    <main className="min-h-screen flex items-center justify-center px-4" style={{backgroundColor:'var(--color-surface)'}}>
      <div className="w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <h1 className="font-bold" style={{fontFamily:FONT_DISPLAY,fontSize:'28px',color:NAVY}}>Reset Password</h1>
        ...
        <div style={{backgroundColor:'#fff',border:BORDER,borderRadius:'16px',boxShadow:'0 4px 12px rgba(13,31,60,0.05)',padding:'24px'}}>
          {invalidLink ? (
            <div className="text-center space-y-4">
              ...
              <Link to="/forgot-password" className="block text-sm font-semibold" style={{color:NAVY}}>Request a new link</Link>
            </div>
          ) : done ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{color:'#374151'}} htmlFor="reset-email">Email</label>
                <input id="reset-email" type="email" className="input-field" value={form.email} onChange={set('email')} required/>
                {errors.email && <p className="text-xs mt-1" style={{color:'#dc2626'}}>{errors.email[0]}</p>}
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{color:'#374151'}} htmlFor="reset-password">New Password</label>
                <input id="reset-password" type="password" className="input-field" value={form.password} onChange={set('password')}
              ...
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{color:'#374151'}} htmlFor="reset-password-confirm">Confirm New Password</label>
                <input id="reset-password-confirm" type="password" className="input-field" value={form.password_confirmation} onChange={set('password_confirmation')} required minLength={8}/>
