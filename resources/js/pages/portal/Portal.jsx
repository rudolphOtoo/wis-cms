...
export default function Portal() {
  ...
  return (
    <div className="min-h-screen" style={{backgroundColor:'var(--color-surface)'}}>
      ...
      {/* Top bar */}
      <header className="shadow-sm" style={{backgroundColor:NAVY}}>
        <div className="flex justify-between items-center max-w-5xl mx-auto px-6 h-16">
          <div className="flex items-center gap-3">
            <picture>
              ...
              <img src="/images/wis-logo.png" alt="Wesleyan International Society Logo" width={32} height={32} className="w-8 h-8 object-contain" />
            </picture>
            <h1 className="font-bold" style={{fontFamily:FONT_DISPLAY,fontSize:'22px',color:'var(--color-gold-light)'}}>WIS-CMS</h1>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        <section className="mb-8">
          <h2 className="font-bold mb-3" style={{fontFamily:FONT_DISPLAY,fontSize:'32px',color:NAVY}}>
            Welcome back, {profile?.full_name?.split(' ')[0]}
          </h2>
          <div className="flex flex-wrap gap-3 items-center">
            <span className="inline-flex items-center rounded-full" style={{padding:'4px 12px',fontSize:'12px',fontWeight:600,backgroundColor:'rgba(0,36,82,0.05)',color:NAVY,border:'1px solid rgba(0,36,82,0.1)'}}>
        ...
        <div className="mb-8" style={{borderBottom:BORDER}}>
              ...
              return (
                <button key={t.k} onClick={() => setTab(t.k)}
                        className="relative transition-colors" style={{padding:'16px 8px',fontSize:'14px',
                                fontWeight: active ? 700 : 600, color: active ? NAVY : MUTED}}>
                  {t.label}
        ...
        {tab === 'giving' && giving && (
          <div className="space-y-6">
            {/* Hero */}
            <div className="relative overflow-hidden" style={{borderRadius:'16px',padding:'32px',backgroundColor:NAVY,boxShadow:'0 4px 12px rgba(13,31,60,0.05)'}}>
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <h3 style={{fontFamily:FONT_DISPLAY,fontSize:'48px',fontWeight:700,color:'white',lineHeight:1}}>{fmt(giving.total)}</h3>
                </div>
            ...
            {giving.total === 0 ? (
              <div className="text-center py-10" style={{...cardBase, padding:'40px'}}>
                ...
                <div style={{fontSize:'14px',fontWeight:600,color:NAVY}}>No giving recorded for {givingYear}</div>
              </div>
            ) : (
              <>
                {/* Category cards */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {giving.by_category.map((cat, i) => (
                    <div key={cat.category} className="flex items-center gap-5 transition-colors group" style={{...cardBase, padding:'24px'}}>
                      ...
                      <div>
                        <p className="uppercase" style={{fontSize:'12px',fontWeight:700,color:MUTED}}>{cat.category} ({cat.count})</p>
                        <p style={{fontFamily:FONT_DISPLAY,fontSize:'24px',fontWeight:600,color:NAVY}}>{fmt(cat.total)}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{...cardBase, overflow:'hidden'}}>
                  <div className="flex justify-between items-center" style={{padding:'24px 32px',borderBottom:BORDER}}>
                    <h4 style={{fontFamily:FONT_DISPLAY,fontSize:'20px',fontWeight:600,color:NAVY}}>My Transactions</h4>
                  </div>
                      ...
                      <thead>
                        <tr style={{backgroundColor:'#f8f9fc'}}>
                          {['Date','Category','Amount (GHS)','Reference'].map(h => (
                            <th key={h} className="uppercase tracking-wider" style={{padding:'16px 32px',fontSize:'12px',fontWeight:700,color:MUTED}}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {giving.transactions.map(t => (
                          <tr key={t.id} className="transition-colors" style={{borderTop:BORDER}}
                            ...
                            <td style={{padding:'20px 32px',fontSize:'15px',fontWeight:700,color:NAVY}}>{fmt(t.amount)}</td>
                            <td style={{padding:'20px 32px',fontSize:'13px',color:MUTED}}>{t.reference ?? '—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        {tab === 'attendance' && attendance && (
          <div className="space-y-6">
            <div className="text-center" style={{...cardBase, padding:'32px'}}>
              <div style={{fontFamily:FONT_DISPLAY,fontSize:'48px',fontWeight:700,color:NAVY,lineHeight:1}}>{attendance.total_present}</div>
              <div style={{fontSize:'14px',color:MUTED,marginTop:'8px'}}>Services attended (all time)</div>
            </div>
            <div style={{...cardBase, overflow:'hidden'}}>
              <div style={{padding:'16px 24px',backgroundColor:'#f8f9fc',borderBottom:BORDER}}>
                <h3 style={{fontSize:'16px',fontWeight:700,color:NAVY}}>Recent Attendance</h3>
              </div>
              {attendance.recent.length === 0 ? (
                <div className="text-center" style={{padding:'32px',fontSize:'14px',color:PLACEHOLDER}}>No attendance recorded yet</div>
              ) : (
                <div>
                  {attendance.recent.map(r => (
                    <div key={r.id} className="flex items-center justify-between" style={{padding:'16px 24px',borderTop:BORDER}}>
                      <span style={{fontSize:'14px',fontWeight:600,color:'#191c1e'}}>{r.service}</span>
                      <span style={{fontSize:'14px',color:MUTED}}>{r.date}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div style={{...cardBase, padding:'24px'}}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {[
                ...
                <div key={label}>
                  <div className="uppercase tracking-wider mb-1" style={{fontSize:'12px',fontWeight:700,color:PLACEHOLDER}}>{label}</div>
                  <div className={cap ? 'capitalize' : ''} style={{fontSize:'15px',fontWeight:500,color:'#191c1e'}}>{value || '—'}</div>
                </div>
              ))}
            </div>
            <p className="mt-6 pt-4" style={{fontSize:'12px',color:PLACEHOLDER,borderTop:BORDER}}>
