...
class DemoDataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        for ($week = 0; $week < 10; $week++) {
            $sunday = now()->startOfWeek()->subWeeks($week)->next(\Carbon\Carbon::SUNDAY);
        ...
        for ($month = 5; $month >= 0; $month--) {
            ...
            for ($sunday = 0; $sunday < 4; $sunday++) {
                $sundayDate = $monthDate->copy()->startOfMonth()->next(\Carbon\Carbon::SUNDAY)->addWeeks($sunday);
                if ($sundayDate->isFuture() || $sundayDate->month !== $monthDate->month) continue;
